...
export function estimateBodyFat(
  ...
  const bodyFat =
    1.2 * bmi + 0.23 * age - 10.8 * (isMale ? 1 : 0) - 5.4;
...
export function calculateProjectedGoalDate(
  ...
  const weightToLose = currentWeight - targetWeight;
  if (weightToLose <= 0) return new Date();
...
export function generateInsight(
  ...
  if (!goalDate) {
    return 'Set a target month to unlock projections.';
  }
  ...
  if (projectedDate && projectedDate <= goal) {
    const daysAhead = Math.round(
      (goal.getTime() - projectedDate.getTime()) /
        (1000 * 60 * 60 * 24)
    );
  ...
  if (requiredRate && requiredRate > weeklyRate) {
    const diff = requiredRate - weeklyRate;
    return `You're about ${diff.toFixed(
      1
    )} kg/week behind. A small activity or calorie adjustment can help.`;
  }
